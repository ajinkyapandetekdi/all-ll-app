...
function StartLearn() {
  ...
  const playAudio = () => {
    interactCall("playAudio", "startlearn", "play", "DT");
    set_temp_audio(new Audio(content[sel_lang].audio));
  };
  // console.log(isAudioPlay);

  const pauseAudio = () => {
    interactCall("pauseAudio", "startlearn", "pause", "DT");
    if (temp_audio !== null) {
      temp_audio.pause();
      setFlag(!false);
    }
  ...
  const learnAudio = () => {
    if (temp_audio !== null) {
      temp_audio.play();
      interactCall("learnAudio", "startlearn", "play", "DT");
      setFlag(!flag);
  ...
  const newSentence = () => {
    interactCall("newSentence", "startlearn", "play", "DT");
    navigate(0);
  };
  useEffect(() => {
    learnAudio();
